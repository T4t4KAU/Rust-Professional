            ...
            for (city, city_list) in cities {
                if let Some(list) = city_list.as_array() {
                    for item in list {
                        ...
                        inner_map.entry(item.as_str().unwrap().to_string()).or_insert(Vec::new()).push(city.clone());
                        inner_map.entry(city.to_string()).or_insert(Vec::new()).push(item.as_str().unwrap().to_string());
                    }
                }
            }
        }
